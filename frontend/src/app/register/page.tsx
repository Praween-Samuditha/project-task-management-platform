        ...
        <div style={{ width: "100%", maxWidth: 1000, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 40 }}>
          ...
          <div style={{ flex: 1, maxWidth: 500 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
              {[
                { icon: "⚡", text: "Create unlimited projects and tasks" },
                { icon: "🔒", text: "Enterprise-grade security built-in" },
                { icon: "🚀", text: "Ship code 40% faster with your team" }
              ].map(feature => (
                <div key={feature.text} style={{ display: "flex", alignItems: "center", gap: 16 }}>
                  <div style={{ width: 40, height: 40, borderRadius: "50%", background: "rgba(255,255,255,0.1)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18 }}>
                    {feature.icon}
                  </div>
                  <p style={{ fontSize: 15, color: "#DFE1E6", fontWeight: 500, margin: 0 }}>{feature.text}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={{ width: "100%", maxWidth: 440 }}>
            <div style={{ background: "#161A1D", borderRadius: 8, padding: "40px", border: "1px solid #2C333A", boxShadow: "0 20px 50px rgba(0,0,0,0.5)" }}>
              <h2 style={{ fontSize: 24, fontWeight: 600, color: "#fff", marginBottom: 32 }}>Sign Up</h2>
              
              <form onSubmit={handleSubmit(onSubmit)} style={{ display: "flex", flexDirection: "column" as const, gap: 16 }}>
                
                <div style={{ display: "flex", gap: 12 }}>
                  <div style={{ flex: 1 }}>
                    <label style={{ display: "block", color: "#8A94A5", fontSize: 12, fontWeight: 700, marginBottom: 8, textTransform: "uppercase", letterSpacing: "0.5px" }}>First Name</label>
                    <input type="text" placeholder="John" style={inputStyle} {...register("firstName")}
                    ...
                    {errors.firstName && <p style={{ fontSize: 13, color: "#FF5630", marginTop: 6, fontWeight: 500 }}>{errors.firstName.message}</p>}
                  </div>
                  <div style={{ flex: 1 }}>
                    <label style={{ display: "block", color: "#8A94A5", fontSize: 12, fontWeight: 700, marginBottom: 8, textTransform: "uppercase", letterSpacing: "0.5px" }}>Last Name</label>
                    <input type="text" placeholder="Doe" style={inputStyle} {...register("lastName")}
                      onFocus={e => { e.target.style.borderColor = atlassianBlue; e.target.style.background = "rgba(255,255,255,0.1)"; }}
                      onBlur={e => { e.target.style.borderColor = "rgba(255,255,255,0.2)"; e.target.style.background = "rgba(255,255,255,0.05)"; }} />
                    {errors.lastName && <p style={{ fontSize: 13, color: "#FF5630", marginTop: 6, fontWeight: 500 }}>{errors.lastName.message}</p>}
                  </div>
                </div>
                ...
                <button
                  ...
                  disabled={loading}
                  style={{
                    width: "100%", height: 48, background: loading ? "#42526E" : atlassianYellow,
                    color: "#172B4D", border: "none", borderRadius: 4,
              ...
              <div style={{ textAlign: "center", marginTop: 32 }}>
                <p style={{ color: "#8A94A5", fontSize: 14 }}>
                  ...
                  <Link href="/login" style={{ color: atlassianBlue, fontWeight: 600, textDecoration: "none" }}
